...
def order_rate_over_time(params, Sdk) :
    ...
    rate_of_change = pd.DataFrame(rate_list, index=[i * WINDOW for i in range(0, len(rate_list))], columns=["Orders"])
    rate_of_change.plot(color="r", linewidth=0.5, marker='o', markersize=3, markeredgecolor='black', markeredgewidth=0.5, markerfacecolor='red', y="Orders", alpha=0.5)
    plt.title('Order Rate OT ({}c)'.format(WINDOW))
    plt.xlabel('Periods')
